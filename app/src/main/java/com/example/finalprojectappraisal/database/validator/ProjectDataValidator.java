...
//        }
        ...
        if (project.getNumberOfFloors() != null && project.getNumberOfFloors() < 0) {
            errors.add("Number of floors cannot be negative");
        }

        if (project.getFloorNumber() != null && project.getFloorNumber() < 0) {
            errors.add("Floor number cannot be negative");
        }

        if (project.getNumberOfRooms() != null && project.getNumberOfRooms() < 0) {
            errors.add("Number of rooms cannot be negative");
        }

        if (project.getRegisteredArea() != null && project.getRegisteredArea() < 0) {
            errors.add("Registered area cannot be negative");
        }

        if (project.getGrossArea() != null && project.getGrossArea() < 0) {
